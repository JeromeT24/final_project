...
fig = px.choropleth(
    ...
    title="World Map showing Prostate cancer New Cases in 2019",
)
...
fig1 = px.histogram(
    ...
    labels={"country": "Country", "new_cases": "Number of new cases"},
    text_auto=True,
    title="Ten countries with the least new cases of prostate cancer in 2019",
)

# Center-align the title
fig1.update_layout(
    title={
        "text": "Ten countries with the least new cases of prostate cancer in 2019",
        "x": 0.5,  # Centers the title
        "xanchor": "center",
        "yanchor": "top",
        "y": 0.99,  # Adjusts the vertical position of the title
...
fig3 = px.histogram(
    ...
    labels={"country": "Country", "number_of_deaths": "Number of deaths"},
    text_auto=True,
    title="Ten countries with the least deaths from prostate cancer in 2019",
)

# Center-align the title and add space between the graph and the title
fig3.update_layout(
    title={
        "text": "Ten countries with the least deaths from prostate cancer in 2019",
        "x": 0.5,  # Centers the title
        "xanchor": "center",
        "yanchor": "top",
        "y": 0.99,  # Adjusts the vertical position of the title
...
### List of 10 countries with hign number of prostate cancer death

## Top 10 countries with highest number of deaths from prostate cancer in 2019
...
top_10_deaths.insert(0, "Sn", range(1, len(top_10_deaths) + 1))

# Reset index to remove the original index
top_10_deaths.reset_index(drop=True, inplace=True)

# Display the table as a nicely formatted string
print("Top 10 Prostate Cancer Deaths by country in 2019\n")
print(top_10_deaths.to_string(index=False))
```
### Plot of 10 countries with hign number of prostate cancer death

## Plot for Top 10 coutries with the highest number of deaths from prostate cancer in 2019
...
prostate_cancer[["country_code", "country", "new_cases", "number_of_deaths"]]
```
...
```{python}
prostate_cancer_subset = prostate_cancer.query("country in ('Canada', 'Sweden', 'Zimbabwe', 'Taiwan', 'Bolivia', 'Tanzania', 'Italy', 'China', 'India')")
```
...
```{python}
px.histogram(
    prostate_cancer_subset,
    x="country",
    y="number_of_deaths",
    text_auto=True,
    title="New cases of Prostate Cancer in Canada, Bolivia, Taiwan, Sweden and Zimbabwe 2019",
)
```
